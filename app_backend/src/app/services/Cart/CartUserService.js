...
export default class CartUserService {
    ...
    async updateItemQuantity(userId, productVariantId, quantity){

        const cart = await Cart.findOne({ where: { user_id: userId, is_active: true } });

        if (!cart) {
            return {status: 422, message: 'Active cart not found.' }
        }

        const cartItem = await CartItem.findOne({ where: { cart_id: cart.id, product_variant_id: productVariantId } });
        ...
        if (quantity <= 0) {
            ...
            const remainingItems = await CartItem.findAll({ where: { cart_id: cart.id } });
            if (remainingItems.length === 0) {
                cart.total_cart_value = 0;
                await cart.save();
                return {status: 200, message: 'Item removed. Cart is now empty.'}
        ...
        cartItem.price = parseFloat(variant.price) * quantity;
        cartItem.shipping_charge = variant.shipping_charge;
        await cartItem.save();
        ...
        return {status: 200, message: 'Quantity updated successfully', data: cartItem}
    }
